...
/**
 * Feeding an item to a player (by click or by drifting into it) applies its fixed per-type hp delta, then the
...
export function eat(itemType: ItemType, playerId: string): ItemInteraction {
  return { hpDeltas: [{ playerId, amount: FRENZY.itemEffects[itemType] }], consumed: true };
}

/**
 * Effect pickup: grants a timed effect to a target and removes the item, optionally healing some hp at the
 * same time (`hp`). The engine reports an `effectGranted` (not `eaten`) so eat FX/stats stay untouched, while
 * any `hp` rides along as a plain hp delta. Shared by shield/vitamin/easterEgg.
 */
export function grantEffect(
  ...
  return {
    hpDeltas: hp === 0 ? [] : [{ playerId, amount: hp }],
    consumed: true,
