...
/**
 * Rock: clicking it does nothing (hp delta 0) but removes it, while a falling rock that bonks a Pokémon
...
export const rockBehavior: ItemBehavior = {
  ...
  onCollide: (_item, player) => ({
    hpDeltas: [{ playerId: player.id, amount: FRENZY.collision.rockDamage }],
    consumed: true,
  }),
};
