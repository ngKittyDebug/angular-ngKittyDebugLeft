...
/**
 * Rolls the mushroom's random integer hp delta within the configured range. Driven by the injected `rng` so
...
/**
 * Mushroom: a gamble — eating it (by click or collision) rolls a random integer hp delta within the configured
...
export const gambleBehavior: ItemBehavior = {
  onClick: (_item, clickerId, _state, _nudgeX, rng = Math.random) => ({
    hpDeltas: [{ playerId: clickerId, amount: gambleDelta(rng) }],
    consumed: true,
  }),
  onCollide: (_item, player, _state, rng = Math.random) => ({
    hpDeltas: [{ playerId: player.id, amount: gambleDelta(rng) }],
    consumed: true,
  }),
};
