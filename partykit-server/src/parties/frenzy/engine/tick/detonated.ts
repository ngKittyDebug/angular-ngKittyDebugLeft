import { FRENZY } from '@game/frenzy/config';
import type { DetonatedEvent, Item } from '@game/frenzy/types';

import type { HpDelta } from '../item-behaviors';

/**
 * A bomb blast as a client-facing event: FX + sound for everyone. The damaged-but-alive hpes reconcile on
 * the next snapshot; faints arrive as their own events from applyHpDeltas.
 */
export function detonated(item: Item, hpDeltas: readonly HpDelta[]): DetonatedEvent {
  return {
    type: 'detonated',
    itemId: item.id,
    x: item.x,
    y: item.y,
    radius: FRENZY.bomb.blastRadius,
    playerIds: hpDeltas.map((delta) => delta.playerId),
  };
}
