...
import { applyImpulses } from '../engine/apply-impulses';
...
describe('applyImpulses', () => {
  ...
  it('adds the kick to the targeted player velocity', () => {
    const next = applyImpulses(stateWith([PLAYER]), [{ playerId: 'p1', ix: 0.02, iy: -0.01 }]);
  ...
  it('caps the resulting speed at blastImpulseMaxFactor × the stage maxSpeed', () => {
    const next = applyImpulses(stateWith([PLAYER]), [{ playerId: 'p1', ix: 5, iy: 0 }]);
  ...
  it('ignores a kick aimed at a player not in the state', () => {
    const next = applyImpulses(stateWith([PLAYER]), [{ playerId: 'ghost', ix: 0.05, iy: 0 }]);
