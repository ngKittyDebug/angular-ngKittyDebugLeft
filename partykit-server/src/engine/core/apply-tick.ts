...
import type { NpcHooks } from '../npc/types';
 ...
 */
export function applyTick<TItemId extends string, TEffectId extends string, TNpcId extends string>(
  game: GameDefinition<TItemId, TEffectId, TNpcId>,
  npc: NpcHooks<TItemId, TEffectId, TNpcId>,
  state: ServerState<TItemId, TEffectId, TNpcId>,
  deltaSeconds: number,
  applyDecay: boolean,
  ...
  const movedItems = moveItems(game, state.items, deltaSeconds);
  // Single-pass partition: floor-survivors keep falling/resting, the just-expired feed resolveLandings below.
  const survivors: Item<TItemId>[] = [];
  const expired: Item<TItemId>[] = [];

  for (const item of movedItems) {
    (item.restMs !== undefined && item.restMs <= 0 ? expired : survivors).push(item);
  }

  // Humans and the NPC move by different rules: humans drift+bounce (movePlayers), the NPC moves via its game's
  // hooks (npc.move). Split by kind, move each, recombine — order is irrelevant for the snapshot.
  const humans = livePlayers.filter(
    (player): player is Player<TEffectId, TNpcId> => !isNPC(player),
  );
  const npcs = livePlayers.filter((player): player is NpcPlayer<TEffectId, TNpcId> =>
    isNPC(player),
  );
  const movedHumans = movePlayers(game, humans, deltaSeconds);
  const movedNpcs = npc.move(npcs, survivors, deltaSeconds, state.tick);

  let working: ServerState<TItemId, TEffectId, TNpcId> = {
    ...state,
    items: survivors,
  ...
  if (game.playerCollision.enabled) {
    ...
    const separation = separatePlayers(game, separable);

    working = { ...working, players: [...separation.players, ...heldNpcs] };

    const bumped = applyBumpDamage(game, working, separation.bumps);

    working = applyImpulses(bumped.state, separation.impulses);
    events.push(...bumped.events);
  }

  // Lift owner-immunity from emitted items that have separated (an emitted explosive that left its emitter's blast
  // radius), so the next collision/landing pass can detonate or strike the ex-owner instead of phasing through.
  working = { ...working, items: armEmittedItems(game, working.items, working.players) };

  const collisions = resolveCollisions(game, working, rng, now);

  working = collisions.state;
  events.push(...collisions.events);

  const landings = resolveLandings(game, working, expired);

  working = landings.state;
  events.push(...landings.events);

  if (applyDecay) {
    const decay = applyDecayStep(game, working);
  ...
  const npcBlasts = npc.applyBlasts(working);
  ...
  working = { ...working, players: npc.coolAnger(working.players) };
  ...
  if (events.some((event) => event.type === 'fainted')) {
    const faints = events.filter(
      (event): event is FaintedEvent<TItemId> => event.type === 'fainted',
    );
