...
describe('applyClick', () => {
  it('returns state unchanged when item is missing', () => {
    ...
    const { state: next, events } = applyClick(FRENZY_DEFINITION, state, PLAYER.id, 'gone');
  ...
  it('returns state unchanged when player is unknown', () => {
    ...
    const { state: next, events } = applyClick(FRENZY_DEFINITION, state, 'ghost', 'i1');
  ...
  it('applies food delta and emits eaten event', () => {
    ...
    const { state: next, events } = applyClick(FRENZY_DEFINITION, state, PLAYER.id, 'i1');
  ...
  it('does not emit evolved when stage stays the same', () => {
    ...
    const { events } = applyClick(FRENZY_DEFINITION, state, PLAYER.id, 'i1');
  ...
  it('shoving a bomb adds an inertial impulse to its drift without eating it or changing hp', () => {
    ...
    const { state: next, events } = applyClick(FRENZY_DEFINITION, state, PLAYER.id, 'i1', -0.1);
  ...
  it('a shove on a bomb without a budget leaves clicksLeft undefined', () => {
    ...
    const { state: next } = applyClick(FRENZY_DEFINITION, state, PLAYER.id, 'i1', -0.1);
  ...
  it('accumulates repeated shoves and caps the bomb drift at maxDriftSpeed', () => {
    ...
    const { state: next } = applyClick(FRENZY_DEFINITION, state, PLAYER.id, 'i1', 1);
