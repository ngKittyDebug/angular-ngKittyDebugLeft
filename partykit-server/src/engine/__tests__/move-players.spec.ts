...
import { movePlayers } from '../core/tick/move-players';
...
describe('movePlayers', () => {
  it('advances a drifting player by its velocity', () => {
    ...
    const [moved] = movePlayers(FRENZY_DEFINITION, [drifting], 1);
  ...
  it('keeps an oversized body further from the wall than the drift zone alone', () => {
    ...
    expect(movePlayers(FRENZY_DEFINITION, [wide], 1)[0].x).toBeCloseTo(inset, 5);
    expect(movePlayers(FRENZY_DEFINITION, [small], 1)[0].x).toBeCloseTo(0.05, 5);
  ...
  it('damps a hard bounce (keeps speed above the floor)', () => {
    ...
    const [bounced] = movePlayers(FRENZY_DEFINITION, [fast], 1);
  ...
  it('bites harder on the floor than the side walls', () => {
    ...
    const [bounced] = movePlayers(FRENZY_DEFINITION, [intoFloor], 1);
  ...
  it('floors a soft bounce back up to the stage cruising speed (no stall)', () => {
    ...
    const [bounced] = movePlayers(FRENZY_DEFINITION, [slow], 1);
