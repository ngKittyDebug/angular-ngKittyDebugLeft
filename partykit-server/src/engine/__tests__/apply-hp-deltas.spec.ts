...
import { applyHpDeltas } from '../core/apply-hp-deltas';
...
describe('applyHpDeltas', () => {
  it('returns the same state reference for an empty delta list', () => {
    ...
    const result = applyHpDeltas(FRENZY_DEFINITION, state, []);
  ...
  it('adds hp without an event when no stage threshold is crossed', () => {
    ...
    const result = applyHpDeltas(FRENZY_DEFINITION, state, [
      { playerId: 'p1', amount: 10, source: 'item' },
    ]);
  ...
  it('clamps at zero, removes the player and emits fainted', () => {
    ...
    const result = applyHpDeltas(FRENZY_DEFINITION, state, [
      { playerId: 'p1', amount: -15, source: 'item' },
    ]);
  ...
  it('applies collateral effects to several players at once', () => {
    ...
    const result = applyHpDeltas(FRENZY_DEFINITION, state, [
      { playerId: 'p1', amount: 10, source: 'item' },
      { playerId: 'p2', amount: -20, source: 'item' },
    ]);
  ...
  it('accumulates multiple deltas aimed at the same player', () => {
    ...
    const result = applyHpDeltas(FRENZY_DEFINITION, state, [
      { playerId: 'p1', amount: 30, source: 'item' },
      { playerId: 'p1', amount: -10, source: 'item' },
    ]);
  ...
  it('clamps a positive delta at the HP ceiling', () => {
    ...
    const result = applyHpDeltas(FRENZY_DEFINITION, state, [
      { playerId: 'p1', amount: 100, source: 'item' },
    ]);

    expect(result.state.players[0].hp).toBe(FRENZY.maxHp);
  });

  it('keeps a warded player UNTOUCHED — a stale-high stage (decay skips stage recompute) is not demoted', () => {
    // hp 150 computes to stage 1 against TEST_BODY's gate (200), but decay left the player at stage 2; the
    // historical shield short-circuit preserved the whole object, so a warded hit must not recompute the stage.
    const stale = player({ hp: 150, stage: 2, effects: [{ kind: 'shield', expiresAt: 10_000 }] });
    ...
    expect(result.state.players[0]).toBe(stale);
    expect(result.state.players[0].stage).toBe(2);
    expect(result.events).toEqual([]);
  });
