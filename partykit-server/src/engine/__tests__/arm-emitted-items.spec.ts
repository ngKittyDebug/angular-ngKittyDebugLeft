...
import { armEmittedItems } from '../core/tick/arm-emitted-items';
...
describe('armEmittedItems', () => {
  it('leaves a non-emitted item (no ownerId) untouched', () => {
    ...
    const [result] = armEmittedItems(FRENZY_DEFINITION, [item], [makePlayer()]);
  ...
  it('keeps a bomb un-armed while it still overlaps its owner body', () => {
    ...
    const [result] = armEmittedItems(FRENZY_DEFINITION, [item], [owner]);
  ...
  it('arms an emitted item whose owner has vanished — no one left to protect', () => {
    ...
    const [result] = armEmittedItems(
      FRENZY_DEFINITION,
      [item],
      [makePlayer({ id: 'someone-else' })],
    );
  ...
  it('keeps a non-bomb emitted item un-armed while overlapping its owner, arms once clear', () => {
    ...
    const [overlapResult] = armEmittedItems(FRENZY_DEFINITION, [overlapping], [owner]);
    const [clearResult] = armEmittedItems(FRENZY_DEFINITION, [clear], [owner]);
