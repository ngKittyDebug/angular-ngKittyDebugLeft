...
- **Gitignore boundaries:** `.planning/`, `docs/agents/`, `docs/adr/`, `CONTEXT.md` and the local part of `.claude/` (settings, worktrees, `codebase-audit-workspace`) are gitignored — edits there live only locally. **Committed:** `CLAUDE.md`, `docs/`, `.agents/skills/`, `skills-lock.json`, `.claude/commands/`, `.claude/skills/` (cloud routines only see what's in the repo). Committed paths go through the normal flow: commit → PR → push.
