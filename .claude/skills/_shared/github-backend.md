...
| Close a fixed/irrelevant issue                | `update_issue` (state=closed) + `add_issue_comment` with the per-item verification — same bar as the gh path: marker-carrying issues only, every checklist path re-verified inside the scanned scope                                                                                                                                                |
