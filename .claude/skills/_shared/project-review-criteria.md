...
| `pokemon-tamagotchi` converter берёт только `evolves_to[0]` (линейная primary-ветвь); ветвящиеся цепочки (Eevee и т.п.) не дают игроку выбор формы.     | `linear-evolution-first-branch` | Сознательное упрощение скоупа tamagotchi: полная механика выбора ветви — отдельная продуктовая задача. Зафиксировано в issue #255. |
