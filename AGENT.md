...
**Не выдумывать новые alias на ходу.** Если нужно делиться кодом между Angular и `partykit-server/` — расширять `shared-game/` под `@game/*`. Подпапка `shared-game/<game>/` оправдана **ТОЛЬКО** когда код потребляют **обе** стороны — и клиент, и `partykit-server/` (реальный wire/логический контракт, как `frenzy`: типы + тюнинг-конфиг + детерминированные хелперы, которые крутят обе стороны); общий между играми код — `shared-game/common/`, когда реально появится. **Клиентская игра (без своей partykit-партии — напр. бой против бота, считаемого в браузере) живёт ЦЕЛИКОМ в своей фиче** — образец `pokemon-tamagotchi` и `pokemon-battle`: типы в `data/models/`, движок/логика в `data/services/`. Не парковать реализацию односторонней игры в `shared-game/`: алиас `@game` — слой контракта, а не свалка «игрового» кода. Если внутри Angular — расширять `@core/*`, `@features/*`, `@shared/*`.
